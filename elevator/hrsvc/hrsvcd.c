...
#define HRSVC_UNIX_SOCK "./var/run/svcd.sock"
#endif
...
static int hrsvc_load_service(const char *directory) {
    ...
    if (stat(directory, &st) || !S_ISDIR(st.st_mode)) {
        printf("it's not a good directory ...:%s\n", directory);
        return -1;
    }
    ...
    while ((entry = readdir(dir))) {
        ...
        for (svc->nargs = 0; svc->nargs < MAX_ARGS && svc->argv[svc->nargs] != NULL; svc->nargs++)
            ;
...
static void _hrsvcd_service_start(struct hrsvc *svc) {
    ...
    if (pid == 0) {  // child pid
        ...
        for (int i = 0; svc->argv[i]; i++) {
            printf("exec %d: -> %s\n", i, svc->argv[i]);
        }
        // service process
        // mxp, 20240513, do not use execve, pass env to child process
        if (execvp(svc->argv[0], (char **)svc->argv) < 0) { // execve
...
static void _hrsvcd_service_stop(struct hrsvc *svc) {
    ...
    // mark it as disabled, so it will will not auto restart when stopped
    svc->flags |= HRSVC_DISABLED;
...
static void _hrsvcd_service_class_start(const char* class) {
    struct j2sobject *n = NULL;
    struct hrsvc *svc = NULL;
    ...
    for (;;) {
        ...
        for (int i = 0; i < nr; i++) {
            ...
            if (_priv.svc_fd == e->data.fd) {
                ...
                pipe_fd[0] = -1;
                pipe_fd[1] = -1;
                _hrsvcd_service_class_start("core");
                _hrsvcd_service_class_start("main");
                _hrsvcd_service_class_start("default");
            } else {  // service stream
