...
static int X_CU_LockEnable_getter(struct dm_object* self, struct dm_value* val) {
    ...
    dm_value_set_number(val, _X_CU_LockEnable);
    return 0;
}

static int X_CU_LockEnable_setter(struct dm_object* self, struct dm_value* val) {
    HR_LOGD("%s(%d): this %p, val:%d\n", __FUNCTION__, __LINE__, self, val->val.number);
    _X_CU_LockEnable = val->val.number;
    return 0;
}
...
int dm_Device_init(struct dm_object* parent) {
    ...
    dm_object_new("X_CU_LockEnable", DM_TYPE_NUMBER, X_CU_LockEnable_getter, X_CU_LockEnable_setter, Device);
