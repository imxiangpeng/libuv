...
static struct iot_topic dm_topic_liftfault = {
...
int topic_houqi_liftfault_init(struct iot* iot,const char* public_key, const char* device_name) {
  (void)public_key;
  (void)device_name;
    iot_topic_register(iot, &dm_topic_liftfault);
    return 0;
}
